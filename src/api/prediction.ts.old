...
export async function createPrediction(
  ...
  const optionInserts = options.map((label, index) => ({
    prediction_id: prediction.id,
    label,
    sort_order: index,
  }));

  const { data: predOptions, error: optError } = await supabase
  ...
  return {
    ...
    options: (predOptions || []) as PredictionOption[],
  };
}
...
export async function enterPrediction(
  predictionId: string,
  optionId: string,
  userId: string,
  stakeAmount: number
...
export async function resolvePrediction(
  ...
  const { error: predError } = await supabase
    ...
    .update({
      status: "resolved",
      resolved_outcome: winningOptionId,
