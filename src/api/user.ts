...
function normalizeUser(row: Record<string, unknown>): User {
  return {
    ...
    ageVerified: (row["age_verified"] === true) || (row["ageVerified"] === true),
    gdprConsent: (row["gdpr_consent"] === true) ? true : (row["gdpr_consent"] === false) ? false : null,
    pushToken: readNullableString(row, "pushToken", "push_token"),
    referredBy: readNullableString(row, "referredBy", "referred_by"),
    referralCode: readNullableString(row, "referralCode", "referral_code"),
