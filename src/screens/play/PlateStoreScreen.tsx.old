...
export function PlateStoreScreen() {
  ...
  const handleBuy = async () => {
    ...
    if (success) {
      Alert.alert("Success", "10 Plates added to your account!");
      refreshProfile();
    }
  };
  ...
  const handleRestore = async () => {
    ...
    Alert.alert(
      ...
      restored ? "Your purchases have been restored." : "No previous purchases found."
    );
  ...
  return (
    <View style={styles.container}>
        ...
        {loading ? (
          ...
          >
            <Text style={styles.buyButtonText}>
              {purchasing ? "Processing..." : "Buy 10 Plates"}
            </Text>
          </TouchableOpacity>
        )}
      ...
      <Text style={styles.legalText}>
        Plates are virtual currency for entertainment purposes only.{"
"}
        They cannot be exchanged for real money or transferred outside the app.
      </Text>
    </View>
  );
}
