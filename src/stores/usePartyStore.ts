    ...
    (set, get) => ({
      ...
      createParty: async (input) => {
        ...
        try {
          ...
          set((state) => ({
            ...
            currentPartyMembers: [result.membership as unknown as PartyMemberWithUser],
            isLoading: false,
          }));
          return result;
        } catch (error) {
        ...
        try {
          ...
          set((state) => ({
            parties: state.parties.map((p) =>
              p.party.id === party.id ? { ...p, party } : p
            ),
            currentParty: state.currentParty?.id === party.id ? party : state.currentParty,
            isLoading: false,
          }));
          return party;
        } catch (error) {
      ...
      leaveParty: async (partyId, userId, deviceId) => {
        ...
        try {
          ...
          set((state) => ({
            parties: state.parties.filter((p) => p.party.id !== partyId),
            currentParty: state.currentParty?.id === partyId ? null : state.currentParty,
            currentPartyMembers: state.currentPartyMembers.filter((m) => m.partyId !== partyId),
            isLoading: false,
          }));
        } catch (error) {
          set({ error: error instanceof Error ? error.message : "Unknown error", isLoading: false });
      ...
      setMemberRole: async (partyId, userId, role, deviceId) => {
        ...
        try {
          ...
          set((state) => ({
            currentPartyMembers: state.currentPartyMembers.map((m) =>
              m.partyId === partyId && m.userId === userId ? { ...m, role } : m
            ),
            isLoading: false,
          }));
        } catch (error) {
